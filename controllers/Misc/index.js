...
const sendInvite = async (request, response) => {
  ...
  const event = {
    ...
    location: `https://meetmove-videochat-app.herokuapp.com/${newRoom.roomID}`,
    description: `Meeting date & time: ${eventStartTime.toLocaleString()}
    Meeting chat URL: https://meetmove-videochat-app.herokuapp.com/chat/${newRoom.roomID}`,
    creator: {
...
const mail = async (recipient, content) => {
  ...
  const mailOptions = {
    ...
    html: `<p p > ${content}</p> `, // plain text body
  };
